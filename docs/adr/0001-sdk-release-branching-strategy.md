# ADR 0001: SDK release branching strategy
...
`ecos` 는 사용자가 Go module로 의존하거나 이후 CLI로 설치할 수 있는 SDK다. 따라서 개발 중인 브랜치와 설치 가능한 배포 기준을 분리해야 한다.
...
- SDK/CLI 설치 기준은 `release/*` 브랜치가 아니라 `vX.Y.Z` SemVer 태그로 둔다.
...
작업 흐름은 단순하게 유지된다. 작은 `feat/*`, `fix/*` 브랜치에서 변경을 나눠 진행하고, 검증된 결과만 `main` 으로 통합한다.

배포 기준은 명확해진다. 사용자는 움직이는 브랜치가 아니라 고정된 태그로 SDK/CLI를 설치한다.

## 대안

### Git Flow

`develop`, `release`, `hotfix`, `main` 을 모두 운영하는 방식이다. 여러 명이 동시에 릴리스를 관리할 때는 도움이 되지만, 현재의 개인 SDK 개발 흐름에는 과하다.
